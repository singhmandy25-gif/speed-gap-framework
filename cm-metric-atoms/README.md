cm-metric-atoms/
Verification scripts for Singh 2026z:
"From Quantum Potential to Atomic Structure: Seven Derived Results in D=3 Geometry"
Paper DOI: (to be assigned upon Zenodo upload)
Author: Mandeep Singh | ORCID: 0009-0003-7176-2395
Scripts
File
Verifies
Chapter
verify_all.py
All paper numbers (master script)
Ch1-Ch9
cm_formula.py
E = ½mc²(W⁻¹/³−1) for Z=1-92
Ch7A
q_zero_proof.py
Q=0 for ρ=A/r² in D=3
Ch1-Ch2
strip_geometry.py
t/w=8π/9, m×R, sphere density
Ch3-Ch5
gold_prediction.py
Z_cross = 79 = Gold
Ch8
taylor_check.py
W⁻¹/³ Taylor coefficients (β⁴=0)
Ch7B
Requirements
Python 3.6+
NumPy (for verify_all.py)
SymPy (for taylor_check.py only)
No other dependencies
Quick Start
Bash
Constants Used
Code
License
Open. Reproduce freely. Cite: Singh 2026z.
